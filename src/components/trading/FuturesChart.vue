...
const generateCandles = (count = 150) => {
    const candles = []
    let basePrice = 95200
    ...
    for (let i = 0; i < count; i++) {
        ...
        const volume = Math.random() * 300 + 60
...
// Calculate EMA for additional indicator
const calculateEMA = (period) => {
    ...
    candles.value.forEach((candle, i) => {
        if (i === 0) {
            ...
            const sum = candles.value.slice(0, i + 1).reduce((acc, c) => acc + c.close, 0)
            ema.push(sum / (i + 1))
        } else {
...
const ema20 = computed(() => calculateEMA(20))
...
onMounted(() => {
    interval = setInterval(() => {
        ...
        const newClose = lastCandle.close + (Math.random() - 0.5) * 60
...
<template>
    <div class="w-full h-full bg-[#181A20] flex flex-col select-none">
        ...
        <div class="flex items-center gap-4 px-4 py-2 text-[11px] border-b border-[#2B3139]">
            ...
            <span class="text-[#2962FF]">EMA(20): {{ ema20[ema20.length - 1]?.toFixed(1) || '--' }}</span>
        </div>

        <!-- Chart Area -->
        <div class="flex-1 relative overflow-hidden">
            <svg :viewBox="`0 0 ${chartWidth} ${chartHeight + volumeHeight + 20}`" preserveAspectRatio="none"
                class="w-full h-full">
                <!-- Grid Lines -->
                <g opacity="0.05">
                ...
                <!-- MA Lines -->
                <path :d="generateMAPath(ma7)" fill="none" stroke="#FCD535" stroke-width="1.5" opacity="0.8" />
                <path :d="generateMAPath(ma25)" fill="none" stroke="#E611FF" stroke-width="1.5" opacity="0.8" />
                <path :d="generateMAPath(ma99)" fill="none" stroke="#EAECEF" stroke-width="1.5" opacity="0.6" />
                <path :d="generateMAPath(ema20)" fill="none" stroke="#2962FF" stroke-width="1.8" opacity="0.85" />
                ...
                <!-- Time Axis -->
                <g :transform="`translate(0, ${chartHeight + volumeHeight + padding.top + 8})`">
                    <text v-for="i in 8" :key="'time' + i"
                        :x="padding.left + ((chartWidth - padding.left - padding.right) / 8) * i" y="15" fill="#848E9C"
                        font-size="10" text-anchor="middle">
                        {{ ['01/05', '01/09', '01/12', '01/15', '01/18', '01/21', '01/24', '01/27'][i] }}
                    </text>
                </g>
            </svg>
        </div>
